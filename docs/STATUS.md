...
- Blender: 5.2.1 LTS installed at `/Applications/Blender.app`; scripting verified. `art/blender/ash_iron_character_studio.blend` contains existing prototype baselines and packed approved references. Godot → Blender → Godot geometry/axis/scale round trip verified. This sets up authored modeling; no new sculpt or replacement is claimed. Source folder is excluded from the game importer.
- Current art priority: only the traveler and Bristleback Boar. The world and gameplay remain as they are while moving toward the approved concepts. Smooth shaped surfaces, swept hair, eyelids/lips, patterned cape and folded hood, curved tusks, muzzle details, and layered fur are now in place. Close illustration fidelity still requires authored sculpts, textures, and a more complete animation rig.
