...
- Blender: six saved review cycles under `art/blender/cycles/`. Latest source is **`cycle_06/characters.blend`** with reconstructed faces on all four travelers; hair/outfits and all four animals are preserved from cycle 4. The latest folder includes a traveler lineup, four face closeups and a scout profile. References packed; offline unrigged studies remain excluded from Godot. Continue from cycle 6, not the old full-set generator.
