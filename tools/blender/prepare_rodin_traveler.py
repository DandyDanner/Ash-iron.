...
Without Rodin textures the figure gets the Willow palette as vertex colors by region; the same script accepts a textured export later.
"""
...
report = {'source': SOURCE.name, 'figure': FIGURE, 'figures_in_source': len(seeds), 'source_triangles': source_triangles, 'triangles': triangles,
	...
	'max_influences': max(influences), 'regions': {'arm': int(arm_mask.sum()), 'head': int(head_mask.sum()), 'gear': int(gear_mask.sum())},
	'limitations': ['Generated topology, no manual retopology', 'Flat Willow palette by region; Rodin texture export not yet applied',
