"""Extract/repair/rig Dallon's largest supplied Bellmaw. Blender 5.2.
Run: blender --background --factory-startup --python tools/blender/rig_imported_bellmaw.py -- /path/to/source.glb
...
SOURCE=Path(sys.argv[sys.argv.index('--')+1]) if '--' in sys.argv else Path('/Users/dallonanderson/Downloads/fantasy+creature+3d+model.glb')
OUT=R/'art/blender/bellmaw_import';OUT.mkdir(parents=True,exist_ok=True)
ASSET=R/'assets/creatures';ASSET.mkdir(parents=True,exist_ok=True)
REVIEW=R/'docs/art/rigged-bellmaw';REVIEW.mkdir(parents=True,exist_ok=True)
...
for v in obj.data.vertices:v.co=(v.co-center)*5
...
# Gray unpainted atlas patches get restrained mottled olive, preserving the normal map.
for mat in obj.data.materials:
 bs=mat.node_tree.nodes.get('Principled BSDF');mat.name='Bellmaw textured hide'
 for socket,value in [('Metallic',0),('Roughness',.82)]:
 ...
 pixels=np.empty(len(tex.pixels),dtype=np.float32);tex.pixels.foreach_get(pixels);p=pixels.reshape(-1,4);rgb=p[:,:3]
 maximum=rgb.max(axis=1);minimum=rgb.min(axis=1);sat=(maximum-minimum)/(maximum+1e-5)
 mask=np.clip((.16-sat)/.065,0,1)*np.clip((maximum-.12)/.1,0,1)
 luminance=rgb.mean(axis=1);tint=np.array([.40,.39,.25]);replacement=np.clip((luminance[:,None]*.48+.29)*tint,0,1)
 rgb[:]=rgb*(1-mask[:,None])+replacement*mask[:,None]
 tex.pixels.foreach_set(pixels);tex.update();tex.pack()
...
report={'source_filename':SOURCE.name,'selected':'largest connected creature; other two and backpack removed','vertices':len(obj.data.vertices),'triangles':sum(len(p.vertices)-2 for p in obj.data.polygons),'bones':list(points),'clips':[c for c,n in clips],'source_bounds_dimensions':list(hi-lo), 'exported_bounds_dimensions_m':[max(v.co[i] for v in obj.data.vertices)-min(v.co[i] for v in obj.data.vertices) for i in (0,2,1)],'runtime_scale':2,'limitations':['Generated topology; no manual quad retopology','No foot IK; uneven slopes may slide','Creature face and rear toes retain source asymmetry']}
