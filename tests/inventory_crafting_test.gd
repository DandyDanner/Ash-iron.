...
func run() -> void:
	...
	check(player.bench_requirement().is_empty(), "Hand crafting still requires the old campsite")
	before = player.inventory.slots.duplicate(true)
	player.craft_axe()
	check(player.inventory.slots == before and not is_instance_valid(player.workbench), "Axe crafted without a placed bench")
	...
	check(not is_instance_valid(player.workbench) and player.inventory.count("bench") == 1 and player.inventory.count("stick") == 4 and player.inventory.count("stone") == 2, "Workbench craft did not produce a portable table for its exact cost")
