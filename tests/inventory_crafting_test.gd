...
func run() -> void:
	...
	check(player.bench_requirement().begins_with("Locked"), "Workbench unlocked before stone axe")
	player.craft_axe()
	check(player.inventory.count("stone_axe") == 1 and player.bench_requirement().is_empty() and not is_instance_valid(player.workbench), "Handmade axe failed to unlock workbench")
	...
	check(not is_instance_valid(player.workbench) and player.inventory.count("bench") == 1 and player.inventory.count("stick") == 1 and player.inventory.count("stone") == 0, "Workbench craft did not produce a portable table for its exact cost")
	player.inventory_panel.place_button.pressed.emit()
	await ticks()
	check(is_instance_valid(player.workbench) and player.inventory.count("bench") == 0, "Placing from the backpack failed")
	check(player.inventory_panel.axe_button.disabled, "Owned axe should not be crafted twice")
