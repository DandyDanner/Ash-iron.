...
func _initialize() -> void:
	Profile.storage_path = Paths.path("bellmaw_attacks_review_profile.json")
	Save.storage_path = Paths.path("bellmaw_attacks_review_save.json")
	Save.clear()
	call_deferred("run")
...
func run() -> void:
	DisplayServer.window_set_title("Ash & Iron — Bellmaw attack review")
	...
	camera.make_current()
	player.global_position = bell.global_position + Vector3(3.5, .9, -1.3)
	camera.position = bell.position + Vector3(6, 5.5, 9)
	camera.look_at(bell.position + Vector3(0, 1.5, 0))
	for sample in [["idle", 0.0], ["warn", .92], ["recover", .08], ["recover", .65], ["swipe_warn", .75], ["swipe", .12]]:
		...
		await capture("bellmaw-attack-%s-%s.png" % [sample[0], str(sample[1])])
	var frame := 0
	for phase in [["warn", 24], ["recover", 25], ["swipe_warn", 17], ["swipe", 5], ["swipe_recover", 21], ["idle", 10]]:
		...
		for i in range(phase[1]):
			...
			await process_frame
			RenderingServer.force_draw(false)
			root.get_texture().get_image().save_png(Paths.path("bellmaw-attack-motion-%03d.png" % frame))
			frame += 1
