...
func run() -> void:
	...
	# The widened outer band must deal damage exactly where the visible ring warns.
	bell.set_physics_process(false)
	reset(player, bell)
	player.global_position = Bell.HOME + Vector3(0, 0.9, 6.6)
	await ticks()
	bell._boom()
	check(player.health == 100 - Bell.BOOM_DAMAGE, "Expanded boom missed the new outer band")
	player.health = 100
	player.damage_grace = 0
	player.global_position = Bell.HOME + Vector3(0, 0.9, 7.2)
	await ticks()
	bell._boom()
	check(player.health == 100, "Boom hit beyond its visible 7 m boundary")
