...
func run() -> void:
	...
	check(bell.health == 80 and player.inventory.is_empty(), "Fresh encounter granted equipment or wrong health")
	await ticks(50)
	...
	reset(player, bell)
	await ticks(105)
	check(player.health == 88 and bell.state == "recover", "Boom did not deal exactly twelve damage after its warning")
	await ticks(25)
	check(player.health == 88, "Recovery inflicted repeated contact damage")
	player.open_inventory()
	var paused_time: float = bell.state_time
	await ticks(100)
	check(bell.state_time == paused_time and player.health == 88, "Inventory did not pause encounter")
	...
	await ticks(120)
	check(bell.state == "idle" and bell.health == 80, "Leaving territory failed to reset the encounter")
	...
	await ticks(45)
	check(bell.health == 60, "Spear did not hit Bellmaw for 20")
	...
	player.fire_bow()
	await ticks(25)
	check(bell.health == 45, "Arrow did not hit Bellmaw for 15")
	bell.receive_melee_hit(100, bell.global_position)
	bell.receive_melee_hit(100, bell.global_position)
