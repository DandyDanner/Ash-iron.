...
func run() -> void:
	...
	full_pack.add("wood", 80)
	stock.add("stick", 3)
	stock.add("stone", 2)
	var stored_before: Array = stock.to_data()
	check(not full_pack.craft_across([full_pack, stock], {"stick": 3, "stone": 2}, "stone_spear") and stock.to_data() == stored_before, "Full-pack spear craft spent connected materials")
	player.inventory.add("stick", 3)
	var before: Array = player.inventory.to_data()
	check(not player.craft_recipe("stone_spear").begins_with("Crafted") and player.inventory.to_data() == before, "Spear crafted with missing stone")
	...
	check(player.inventory.count("stone_spear") == 1 and player.inventory.count("stick") == 0 and chest.storage.is_empty(), "Wrong spear cost or connected-storage consumption")
