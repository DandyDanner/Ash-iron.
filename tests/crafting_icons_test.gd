...
func run() -> void:
	...
	check(player.inventory.to_data() == before and not player.workbench.built, "Browsing spent resources or built a bench")
	...
	check(player.workbench.built and panel.craftables.bench.availability.text == "BUILT", "Build action did not update the icon state")
