...
func _environment() -> void:
	...
	sky_material.sky_top_color = Color("5d93b3")
	sky_material.sky_horizon_color = Color("cdd6ba")
	sky_material.ground_horizon_color = Color("b8c19a")
	sky_material.ground_bottom_color = Color("5c6b4a")
	sky_material.sky_curve = 0.18
	sky_material.sun_angle_max = 18.0
	sky_material.sun_curve = 0.12
	sky.sky_material = sky_material
	env.sky = sky
	# Shadows are lit by a cool sky tint; the sun stays near its original energy because the
	# meadow shaders were converted to linear color rather than re-lit to compensate.
	env.ambient_light_source = Environment.AMBIENT_SOURCE_COLOR
	env.ambient_light_color = Color("b9cdd6")
	env.ambient_light_energy = 0.42
	env.reflected_light_source = Environment.REFLECTION_SOURCE_SKY
	env.tonemap_mode = Environment.TONE_MAPPER_ACES
	env.tonemap_exposure = 1.0
	env.tonemap_white = 1.0
	# Forward+ only; the compatibility renderer ignores these and keeps the flat look.
	...
	env.fog_light_color = Color("c4d2c9")
