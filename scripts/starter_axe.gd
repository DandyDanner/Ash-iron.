...
const SWING_ITEMS := ["stone_axe", "copper_axe", "stone_pickaxe", "stone_spear"]
# The forearm's far end in hand space. Swings pivot here so the sleeve stays planted at the
# edge of the view instead of the whole arm sliding around the hand.
const ELBOW := Vector3(-0.02, -0.2, 0.5)
var elapsed := -1.0
...
func set_item(item: String) -> void:
	...
	if not item in SWING_ITEMS:
		cancel_swing()

func start_swing() -> bool:
	## Empty hands throw a short punch; tools swing or thrust.
	if not (selected_item in SWING_ITEMS or selected_item.is_empty()) or elapsed >= 0.0:
		return false
	elapsed = 0.0
	contact_sent = false
...
func swing_progress() -> float:
	return clampf(elapsed / SWING_DURATION, 0.0, 1.0) if elapsed >= 0.0 else 0.0

func cancel_swing() -> void:
	elapsed = -1.0
	position = rest_position
	rotation = Vector3.ZERO
	if selected_item.is_empty() and is_instance_valid(hand):
		hand.set_grip(false)
...
func pose_swing(time: float) -> void:
	if selected_item == "stone_spear":
		...
		return
	if selected_item.is_empty():
		# A short jab: the fist drives forward and inward while the sleeve trails from the
		# lower right, so the arm reads as extended from the shoulder rather than raised.
		var reach := thrust_distance(time) * 0.55
		var k := clampf(reach / 0.21, 0.0, 1.0)
		rotation = Vector3(0.15 * k, 0.2 * k, 0.1 * k)
		position = rest_position + Vector3(-0.16 * k, 0.02 * k, -reach)
		return
	# Wind up, chop down, follow through, recover. The hand orbits the elbow, so the sleeve
	# stays planted at the edge of the view. Pitch only: the head keeps a vertical plane
	# beside the crosshair, which is what makes the point of impact readable.
	var pitch: float
	var lunge := 0.0
	if time < 0:
		pitch = 0.0
	elif time < 0.08:
		pitch = lerpf(0.0, 0.62, smoothstep(0, 0.08, time))
	elif time < CONTACT_TIME:
		var t := smoothstep(0.08, CONTACT_TIME, time)
		pitch = lerpf(0.62, -0.38, t)
		lunge = -0.06 * t
	elif time < 0.32:
		var t := smoothstep(CONTACT_TIME, 0.32, time)
		pitch = lerpf(-0.38, -0.62, t)
		lunge = lerpf(-0.06, -0.03, t)
	else:
		var t := smoothstep(0.32, SWING_DURATION, time)
		pitch = lerpf(-0.62, 0.0, t)
		lunge = lerpf(-0.03, 0.0, t)
	var spin := Basis.from_euler(Vector3(pitch, 0, 0))
	rotation = Vector3(pitch, 0, 0)
	position = rest_position + ELBOW + spin * (-ELBOW) + Vector3(0, 0, lunge)
