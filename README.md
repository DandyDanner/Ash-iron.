...
## Approved crafting progression

**Stone → Copper → Bronze → Iron → Steel.** The first playable stages now cover inventory-made stone tools, earned workbench/furnace unlocks, copper smelting, a permanent Copperworking kit and a copper axe. Bronze, iron equipment and steel remain future stages. See the [craftsman progression plan](docs/CRAFTSMAN_PROGRESSION.md).
...
The Stone/Copper sequence below is playable. Existing iron deposits and furnace processing remain compatible; older saves retain access to their previous recipes and keep all materials.
...
- Left click — with empty hands, a quick punch (3 damage, no effect on trees or rock); swing your equipped axe, or your pickaxe at boulders and iron veins; thrust with a spear; with a bow, hold to draw and release to fire (while the mouse is captured)
...
3. Craft a **stone axe by hand: 3 sticks + 2 stones**, directly in Tab. This permanently unlocks the workbench recipe. A stone pickaxe (3 sticks + 4 stones) and spear (3 sticks + 2 stones) are also made by hand.
4. Spend **6 sticks + 4 stones** on **Craft workbench**. It goes into your backpack; select it, face clear ground, and click **Place workbench here**. Bow, arrows, torch, wood splitting, chest and Explorer Pack recipes need a reachable bench. Crafting both a stone axe and a stone pickaxe unlocks the furnace recipe there. Learned milestones remain earned if tools are stored, dropped or lost.
5. Close the backpack, walk up to a pine, and click ten times to chop it down (a copper axe needs five). Aim at the fallen logs and press **E** to collect five wood into your backpack.
...
7. At the bench, craft a **stone furnace for 10 stones + 2 wood**, then place it like a chest. New furnaces start in **Copper** mode. Mine the three green-flecked copper outcrops around the clearing (west/northwest, north beyond the target, and southwest); each yields four ore. Two ore and one wood make one ingot in **12 seconds**. Furnaces automatically reserve one complete batch from chests within **8 meters**. Manual Load uses your backpack first, then linked chests.
8. Your first completed copper smelt unlocks **Copper fittings** at the bench: **1 copper ingot → 2 fittings**. Use **2 fittings + 2 wood + 2 stones** to fit the **Copperworking kit**. It permanently improves every bench you use, takes no backpack slot, and unlocks the **copper axe: 3 copper ingots + 2 sticks**. Copper axes fell a pine in two swings and deal 14 base melee damage; stone axes remain 10 and spears 20.
9. The backpack shows your next milestone, **LOCKED** recipes and the reason for each lock. Scroll the icon list for later recipes; select an icon to see materials and the craft button. Existing iron supplies can still be processed by selecting **Iron** in an empty furnace. Return loaded ore and take its ingots before switching metal; fuel stays in the furnace. Returning ore pauses auto-feed so it does not immediately reload. Old placed furnaces retain Iron mode and their saved contents/progress.


You can have multiple workbenches. Stand close and press **E** to use a particular bench; opening **Tab** uses the nearest reachable one. Stone axe, pickaxe and spear recipes work by hand; other workshop recipes require a placed bench. **Pick up this workbench** returns it to an empty backpack slot so you can move camp; its connected chests stay in place with their contents. Hand crafting can use nearby linked stock while at a bench, but cannot draw from distant chests.
...
| Stone spear | 3 sticks + 2 stones | Left click for a forward thrust. Practice within 2.8 meters of the target. |
...
| Copper fittings | 1 copper ingot | Produces two fittings after your first copper smelt. |
| Copperworking kit | 2 fittings + 2 wood + 2 stones | Permanent copper tool access at your benches; no output slot needed. |
| Copper axe | 3 copper ingots + 2 sticks | Requires the Copperworking kit; two chops per swing (five swings fell a pine), 14 melee damage. |
| Stone furnace | 10 stones + 2 wood | Placeable smelter. Two selected ore + one wood become one matching ingot every twelve seconds; it keeps working while you are away. |
...
Defeating the boar leaves **one boar hide** to gather with E. It stacks to ten and can be stored or dropped; leather recipes will come later. Both the boar and Bellmaw respawn **two minutes of active play after defeat**, once you are at least **8 meters from their home** and the spawn space is clear. Menus/focus loss pause the timer; saving and quitting preserves the remaining time, with no offline countdown. Each new defeat drops one hide; old loose hides remain collectible. The approved wildlife art direction and later progression are recorded in `docs/art/BRISTLEBACK.md`. Ore, the furnace, and iron ingots are already prototyped; the **stone/copper foundation** is now playable, with bronze next in the material plan.

## Wildlife model preview

Bellmaw and Bristleback use their new models in the existing encounters. The Hollow Wolf and Meadow Buck are animated preview assets; they do not yet spawn in the world. Open `scenes/wildlife_preview.tscn` and run that scene (Command + R on Mac): **1 / 2 / 3** selects boar / deer / wolf, **Left / Right** changes the movement, and **Space** pauses. Drag to orbit and scroll to zoom. The viewer does not touch your saved game.

Editable sources and rebuild commands are in [the creature review](docs/art/CREATURE_RODIN_REVIEW.md).
...
The **Bellmaw** now uses a native-textured Rodin model with 60k triangles, original 2K PBR maps and a 16-bone combat rig. Its existing giant scale and collision remain. [Current creature models and attack motion](docs/art/CREATURE_RODIN_REVIEW.md). It has a broad, four-legged body and an amber throat that swells before its attack. It raises both front paws during a **1.2-second warning**, then slams them down with dust and a boom inside the visible **7-meter ring**. Back outside the ring or put solid rock cover between you and it. Strike during its **1.25-second recovery**. It has **300 health** and approaches at **3.3 m/s**. During recovery it takes **20 spear / 10 axe / 15 arrow damage**; its thick hide halves incoming damage at all other times (rounded down). The boom deals **34 damage**, enough for three hits to defeat a fully healthy traveler. Fifteen perfectly timed spear hits can win; attacks against its guarded hide take thirty. Watch for **SOFT THROAT — strike now!** after the boom. After a slam, standing close on either front flank can provoke an **18-damage paw swipe**: a **0.85-second windup**, fixed facing, and a filled **7.6-meter side sector** show where it will hit. Back away, circle behind or use cover. Its **1.05-second recovery** also exposes the throat; a five-second cooldown and another required slam prevent repeated swipe chains. [Current attack motion and tuning](docs/art/CREATURE_RODIN_REVIEW.md). It returns home and heals when you leave its territory. Panels/focus loss pause it. This daytime prototype does not require a night cycle or lantern.
...
Authored hands use connected palms/fingers, relaxed empty-hand angles, and sleeves anchored between each wrist and the lower camera corner. Tool grips and bow-string contact retain their alignment. See [actual first-person review](docs/art/FIRST_PERSON_HANDS_REVIEW.md).
...
- `Godot --headless --path . --script res://tests/first_person_hands_test.gd` — first-person wrist/shoulder attachment, tool poses and bow nock tracking.
...
- `Godot --headless --path . --script res://tests/native_wildlife_test.gd` — native wildlife materials, rigging, seam weights, grounding and movement.
...
Use your Godot executable in these commands (thirty-three suites):
...
- `Godot --headless --path . --script res://tests/crafting_progression_test.gd` — earned locks, handcrafting, copper mining/settling, smelting and linked supplies, safe metal switching, permanent bench kit, copper axe in both views, saves and legacy compatibility. Add `-- --screenshots=/absolute/output/folder` with graphics enabled for the actual menus and axe.

- `Godot --headless --path . --script res://tests/bellmaw_rig_test.gd` — imported skeleton, normalized skin weights, visible throat binding, walking/return poses, reset and independent warning radius.
...
For current Bellmaw motion captures, run `Godot --path . --script res://tests/bellmaw_rig_preview.gd`. It captures idle/walk/warning/ring views and 84 motion frames into the temporary test folder, then closes its own window.
