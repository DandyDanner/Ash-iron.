...
The **Bellmaw** uses the largest creature from the supplied 3D model, with a 16-bone weighted skeleton, walking/return gait, swelling throat and recovery motion. It retains the doubled body scale; a longer capsule matches the imported torso. [Actual motion preview and source review](docs/art/IMPORTED_MODELS_REVIEW.md). It has a broad, four-legged body and an amber throat that swells before its attack. See the [actual size comparison beside the player](docs/art/BELLMAW_SIZE_REVIEW.md). It raises both front paws during a **1.2-second warning**, then slams them down with dust and a boom inside the visible **6-meter ring**. Back outside the ring or put solid rock cover between you and it. Strike during its **1.25-second recovery**. It has **160 health** and approaches at **3.3 m/s**. During recovery it takes **20 spear / 10 axe / 15 arrow damage**; its thick hide halves incoming damage at all other times (rounded down). The boom deals **34 damage**, enough for three hits to defeat a fully healthy traveler. Eight perfectly timed spear hits can win; attacks against its guarded hide take sixteen. Watch for **SOFT THROAT — strike now!** after the boom. After a slam, standing close on either front flank can provoke an **18-damage paw swipe**: a **0.85-second windup**, fixed facing, and a filled **3.8-meter side sector** show where it will hit. Back away, circle behind or use cover. Its **1.05-second recovery** also exposes the throat; a five-second cooldown and another required slam prevent repeated swipe chains. [Actual attack motion and tuning](docs/art/BELLMAW_ATTACKS_REVIEW.md). It returns home and heals when you leave its territory. Panels/focus loss pause it. This daytime prototype does not require a night cycle or lantern.
...
For current slam/swipe captures, run `Godot --path . --script res://tests/bellmaw_attacks_preview.gd`. It uses temporary saves and records actual runtime poses and motion.
